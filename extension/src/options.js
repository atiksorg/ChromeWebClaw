...
// v5.0 direct tab
const useDirectTabEl = $('use_direct_tab');
// v8.0 token budget
...
async function load() {
  ...
  // v5.0 direct tab
  useDirectTabEl.checked = s.use_direct_tab !== undefined ? !!s.use_direct_tab : true;
  // v8.0 token budget
...
saveBtn.addEventListener('click', async () => {
  ...
  await setSettings({
    ...
    // v5.0 direct tab
    use_direct_tab: useDirectTabEl.checked
  });
...
fetchRemoteBtn.addEventListener('click', async () => {
  ...
  // v5.0 direct tab
  if (c.use_direct_tab !== undefined) useDirectTabEl.checked = !!c.use_direct_tab;
  // v8.0 token budget
...
importFileEl.addEventListener('change', async (e) => {
  ...
  try {
    ...
    if (config.iframe_bypass_enabled !== undefined) iframeBypassEl.checked = !!config.iframe_bypass_enabled;
    ...
    // v4.0 batch
    if (config.action_delay_ms != null) actionDelayEl.value = config.action_delay_ms;
    if (config.max_actions_per_session != null) maxActionsEl.value = config.max_actions_per_session;
    if (config.autonomy_mode) autonomyModeEl.value = config.autonomy_mode;
    // v5.0 direct tab
    if (config.use_direct_tab !== undefined) useDirectTabEl.checked = !!config.use_direct_tab;
    // v8.0 token budget
