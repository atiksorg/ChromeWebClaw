  ...
  switch (msg.kind) {
    ...
    case 'plan_ready':
      if (msg.plan?.type === 'batch') {
        addLine(`📋 план: batch — ${msg.plan.goal || ''}`, 'act');
      } else {
        addLine('📋 план: simple');
      }
      break;
    case 'batch_started':
      addLine(`🔄 batch начат: ${msg.total} элементов`, 'act');
      break;
    case 'batch_progress':
      addLine(`⏳ batch ${msg.current}/${msg.total}: ${msg.itemTitle || msg.itemId}`, 'act');
      break;
    case 'batch_finished':
      if (msg.report) {
        const r = msg.report;
        addLine(`📊 batch: ${r.succeeded || 0}/${r.total || 0} ok, ${r.failed || 0} fail, ${r.skipped || 0} skip`, r.ok ? 'ok' : 'err');
      }
      break;
    case 'confirmation_required':
      addLine(`⚠ подтверждение: ${(msg.items || []).length} элементов — ${msg.goal || ''}`, 'act');
      break;
    case 'paused_for_confirmation':
      addLine(`⏸ пауза: ожидание подтверждения ${(msg.items || []).length} элементов`, 'act');
      break;
    case 'finished':
